                ...
                <div class="row d-flex align-items-center vh-100">
                    ...
                    <div class="col-md-6 offset-1 wow animate__animated animate__bounceInRight" data-wow-duration="2s">
                        <div class="text-start">
                            <form action="handlers/update-logo.php" method="POST" enctype="multipart/form-data">
                                <div class="mb-3">
                                    <label for="exampleInputname" class="form-label text-capitalize">name</label>
                                    <input type="text" name="name" value="<?=$logo['name']?>" class="form-control" id="exampleInputname" aria-describedby="name">
                                </div>
                                <div class="mb-3">
                                    <label for="exampleInputEmail1" class="form-label text-capitalize">image</label>
                                    <input type="file" name="image" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp">
                                    <img class="rounded-circle w-25" src="handlers/upload/logo/<?=$logo['image']?>" alt="<?=$logo['name']?> logo">
                                </div>
                                <input type="hidden" name="logo_id" value="<?=$logo['id']?>">
                                <button type="submit" class="btn text-capitalize rounded-pill mt-2 py-2 px-3">update logo</button>
                            </form>
                        </div>
                    </div>
                </div>
